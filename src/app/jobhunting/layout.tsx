
import ModuleSidebar from '@/components/ModuleSidebar';
import { JOB_MODULES } from '@/lib/data';

export default function JobhuntingLayout({ children }: { children: React.ReactNode }) {
    return (
        <div className="flex min-h-screen bg-[var(--bg)] overflow-hidden">
            <ModuleSidebar modules={JOB_MODULES} path="jobhunting" title="Conseguir el Trabajo" />
            <main className="flex-1 min-w-0 ml-0 md:ml-64 min-h-screen bg-[var(--bg)] overflow-x-hidden">
                {children}
            </main>
        </div>
    );
}
